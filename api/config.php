...
if (!$databaseUrl && !$USE_SUPABASE_REST) {
    ...
    // --- Persistent "remember me" cookie helpers ---
    // Cookie stores base64(uid|expiry|hmac). HMAC = sha256(uid|expiry, SECRET)
    define('REMEMBER_COOKIE_NAME', 'flashaza_remember');
    define('REMEMBER_COOKIE_DAYS', 30);

    // Secret used to sign tokens. Prefer REMEMBER_SECRET env var.
    $REMEMBER_SECRET = trim(getenv('REMEMBER_SECRET') ?: '');
    if ($REMEMBER_SECRET === '') {
        $REMEMBER_SECRET = trim(getenv('SUPABASE_SERVICE_ROLE_KEY') ?: 'flashaza_default_secret');
    }

    function create_remember_token($uid) {
    ...
    function set_remember_cookie($uid) {
        $token = create_remember_token($uid);
        setcookie(REMEMBER_COOKIE_NAME, $token, time() + REMEMBER_COOKIE_DAYS * 24 * 60 * 60, '/', '', isset($_SERVER['HTTPS']), true);
    }
    ...
    function verify_remember_token() {
        global $REMEMBER_SECRET;
        if (empty($_COOKIE[REMEMBER_COOKIE_NAME])) return false;
        $raw = base64_decode($_COOKIE[REMEMBER_COOKIE_NAME], true);
        if ($raw === false) return false;
        $parts = explode('|', $raw);
        if (count($parts) !== 3) return false;
        list($uid, $expiry, $hmac) = $parts;
        if (!ctype_digit((string)$uid) || !ctype_digit((string)$expiry)) return false;
        if (intval($expiry) < time()) return false;
        $data = $uid . '|' . $expiry;
        $expected = hash_hmac('sha256', $data, $REMEMBER_SECRET);
    ...
    // If no active session, try to auto-login via remember cookie
    if (session_status() === PHP_SESSION_NONE) {
        if (!headers_sent()) {
            // do not start session if headers already sent
            session_start();
            if (empty($_SESSION['user_id'])) {
    ...
    // --- Make PHP session cookie persistent ---
    // Number of days to keep PHPSESSID persistent (same as remember period)
    if (!defined('SESSION_COOKIE_DAYS')) define('SESSION_COOKIE_DAYS', REMEMBER_COOKIE_DAYS);
    $session_lifetime = SESSION_COOKIE_DAYS * 24 * 60 * 60;
    // Advise PHP to keep session data longer on server-side
    @ini_set('session.gc_maxlifetime', (string)$session_lifetime);
    // Advise PHP to set cookie lifetime for future session_start calls
    @ini_set('session.cookie_lifetime', (string)$session_lifetime);
    // If a session is already active, refresh the session cookie with an expiry
    if (session_status() !== PHP_SESSION_NONE) {
        if (!headers_sent()) {
            setcookie(session_name(), session_id(), time() + $session_lifetime, '/', '', isset($_SERVER['HTTPS']), true);
        }
    }
